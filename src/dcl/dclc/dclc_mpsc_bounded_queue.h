//
// Created by Dominic Kloecker on 27/06/2026.
//

#ifndef DSL_DCLC_MPSC_BOUNDED_QUEUE_H_
#define DSL_DCLC_MPSC_BOUNDED_QUEUE_H_
...
public:
    ...
    bool push(const T &data) {
    	// Acquire lock to synchronise between producers
    ...
    ~mpsc_bounded_queue_imp() {
        reset();
    }
};

/**
* @brief Multi-producer (locking) single-consumer bounded queue, with a capacity at minimum
 * the requested size.
 *
 * Locks on attempted push
 *
...
class mpsc_bounded_queue : public mpsc_bounded_queue_imp<T, dsl::round_up_pow2(RequestedCapacity)> {
public:
...
#endif  // DSL_DCLC_MPSC_BOUNDED_QUEUE_H_
