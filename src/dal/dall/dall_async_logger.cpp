...
#include "dall_log_writer.h"

namespace dal {
template<size_t QueueCapacity, size_t FlushThreshold>
std::optional<dcl::LifeTimed::Error> AsyncLogger<QueueCapacity, FlushThreshold>::runStart() {
	...
	if (config_.enable_file) {
		...
		if (!log_file_.is_open()) {
			return Error{
				.life_time_rc = RCodes::TRANSITION_FAILED,
...
template<size_t QueueCapacity, size_t FlushThreshold>
std::optional<dcl::LifeTimed::Error> AsyncLogger<QueueCapacity, FlushThreshold>::runStop() {
	stop_.request_stop();
